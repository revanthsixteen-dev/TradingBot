...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    cache_dir = os.path.abspath(config["data_cache_dir"])
    os.makedirs(cache_dir, exist_ok=True)
    data_file = os.path.abspath(os.path.join(cache_dir, f"{safe_symbol}-YFin-data.csv"))
    
    # Path traversal check
    if not data_file.startswith(cache_dir):
        raise ValueError(f"Path traversal attempt detected in symbol: {symbol}")

    data = None
    if os.path.exists(data_file):
        # Only reuse if cache file was updated today to keep it fresh
